...
import { type PyValue, float, fromJson, int } from './values.js'

/** A JSON number is a float if it was written with a point or an exponent. */
function classify(source: string, value: number): PyValue {
 ...
 */
export function fromJsonText(text: string): PyValue {
  const marked = new WeakMap<object, Map<string | number, PyValue>>()
  let sawSource = false

  const revived = JSON.parse(text, function reviver(this: unknown, key, value, context?: { source?: string }) {
  ...
  if (!sawSource) return fromJson(revived)

  const rebuild = (node: unknown, holder?: object, key?: string | number): PyValue => {
    if (typeof node === 'number' && holder) {
      const tagged = marked.get(holder)?.get(key as string | number)
      if (tagged) return tagged
    }
    if (Array.isArray(node)) return { t: 'list', v: node.map((item, i) => rebuild(item, node, i)) }
 ...
 */
export function fromParameterMap(params: Readonly<Record<string, number>>): PyValue {
  const entries = new Map<string, PyValue>()
  for (const [name, value] of Object.entries(params)) entries.set(name, float(value))
  return { t: 'dict', v: entries }
}
