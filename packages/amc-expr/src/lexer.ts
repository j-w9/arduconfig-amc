...
export type TokenType = 'number' | 'string' | 'name' | 'op' | 'eof'

export interface Token {
  readonly type: TokenType
  /** Literal text for ops and names; the parsed body for strings. */
  readonly value: string
  /** Numeric value, and whether the literal was written as an integer. */
  readonly num?: number
...
export function tokenize(source: string): Token[] {
  ...
  while (i < source.length) {
    ...
    if (ch === "'" || ch === '"') {
      ...
      if (i >= source.length) throw new PyError('SyntaxError', `unterminated string at ${start}`)
      i += 1
    ...
    if (/[0-9]/.test(ch) || (ch === '.' && /[0-9]/.test(source[i + 1] ?? ''))) {
      const start = i
      while (i < source.length && /[0-9_]/.test(source[i] as string)) i += 1
      let isInt = true
    ...
    if (!op) throw new PyError('SyntaxError', `unexpected character ${JSON.stringify(ch)} at ${i}`)
