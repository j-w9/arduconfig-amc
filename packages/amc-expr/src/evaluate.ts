...
import { type CompareOp, type Node, parse } from './parser.js'
import {
  NONE,
  PyError,
  type PyValue,
 ...
 */
function exactRational(value: number): { num: bigint; den: bigint } {
...
function pyRound(value: number, digits = 0): number {
  ...
  if (twice > den || (twice === den && quotient % 2n === 1n)) quotient += 1n
...
function pyTrunc(value: number): number {
  return value < 0 ? Math.ceil(value) : Math.floor(value)
}

function numeric(value: PyValue, context: string): number {
  if (isNumeric(value)) return value.v
...
function bothInt(left: PyValue, right: PyValue): boolean {
  const intish = (v: PyValue) => v.t === 'int' || v.t === 'bool'
...
function versionParts(text: string): number[] {
  const core = text.trim().replace(/^v/i, '').split(/[-+]/)[0] as string
...
function equal(left: PyValue, right: PyValue): boolean {
  if (left.t === 'version' || right.t === 'version') {
    if (left.t !== 'version' || right.t !== 'version') return false
    return compareVersions(left.v, right.v) === 0
  }
  if (isNumeric(left) || left.t === 'bool') {
    if (!(isNumeric(right) || right.t === 'bool')) return false
    return numeric(left, '==') === numeric(right, '==')
  }
  if (left.t === 'str' && right.t === 'str') return left.v === right.v
  if (left.t === 'none' && right.t === 'none') return true
  if (left.t === 'list' && right.t === 'list') {
    return left.v.length === right.v.length && left.v.every((item, i) => equal(item, right.v[i] as PyValue))
  }
  if (left.t === 'dict' && right.t === 'dict') {
    if (left.v.size !== right.v.size) return false
...
function ordered(left: PyValue, right: PyValue, op: CompareOp): boolean {
  let sign: number
  if (left.t === 'version' && right.t === 'version') {
...
function contains(needle: PyValue, haystack: PyValue): boolean {
  switch (haystack.t) {
    case 'dict': {
      if (needle.t !== 'str') return false
...
function callFunction(name: string, args: PyValue[]): PyValue {
  const arity = (n: number) => {
    if (args.length !== n) throw new PyError('TypeError', `${name}() takes ${n} argument(s), got ${args.length}`)
  }
  ...
  switch (name) {
    ...
    case 'min': {
      ...
      const pool: PyValue[] = only?.t === 'list' ? [...only.v] : args
      if (pool.length === 0) throw new PyError('ValueError', `${name}() arg is an empty sequence`)
      let best = pool[0] as PyValue
    ...
    case 'round': {
      ...
      const target = args[0] as PyValue
      ...
      if (args.length === 1) return int(pyRound(value))
      const rounded = pyRound(value, numeric(args[1] as PyValue, 'round'))
      return target.t === 'float' ? float(rounded) : int(rounded)
    }
    case 'abs': {
      arity(1)
      const value = args[0] as PyValue
    ...
    case 'len': {
      ...
      if (value.t === 'list') return int(value.v.length)
      if (value.t === 'dict') return int(value.v.size)
    ...
    case 'log': {
      ...
      const value = numeric(args[0] as PyValue, 'log')
      if (value === 0) throw new PyError('ValueError', 'math domain error')
      if (value < 0) throw new PyError('ValueError', 'math domain error')
      if (args.length === 2) return float(Math.log(value) / Math.log(numeric(args[1] as PyValue, 'log')))
    ...
    case 'Version': {
      arity(1)
      const value = args[0] as PyValue
      if (value.t !== 'str') throw new PyError('TypeError', 'Version() expects a string')
...
function evalNode(node: Node, scope: Map<string, PyValue>): PyValue {
  switch (node.kind) {
    case 'num':
    ...
    case 'name': {
      ...
      if (value === undefined) throw new PyError('NameError', `name '${node.id}' is not defined`)
      return value
    }
    case 'list':
    ...
    case 'index': {
      ...
      if (target.t === 'dict') {
        if (key.t !== 'str') throw new PyError('KeyError', String(key.t === 'none' ? 'None' : (key as { v: unknown }).v))
        const found = target.v.get(key.v)
        if (found === undefined) throw new PyError('KeyError', `'${key.v}'`)
        return found
      }
      if (target.t === 'list' || target.t === 'str') {
        ...
        return target.t === 'list' ? (target.v[at] as PyValue) : str(target.v[at] as string)
      }
      throw new PyError('TypeError', `'${typeName(target)}' object is not subscriptable`)
    }

    case 'call': {
      if (node.callee.kind !== 'name') throw new PyError('TypeError', 'only named functions may be called')
      return callFunction(
        node.callee.id,
    ...
    case 'method': {
      ...
      if (target.t !== 'str') throw new PyError('TypeError', `'${typeName(target)}' object has no attribute 'split'`)
      ...
      const separator = args[0] as PyValue
      if (separator.t !== 'str') throw new PyError('TypeError', 'split() expects a string separator')
      if (separator.v === '') throw new PyError('ValueError', 'empty separator')
      return { t: 'list', v: target.v.split(separator.v).map(str) }
    }

    case 'unary': {
      if (node.op === 'not') return bool(!truthy(evalNode(node.operand, scope)))
    ...
    case 'bool-op': {
      ...
      if (node.op === 'and') return truthy(left) ? evalNode(node.right, scope) : left
    ...
    case 'binary': {
      ...
      if (node.op === '+') {
        if (left.t === 'str' && right.t === 'str') return str(left.v + right.v)
        if (left.t === 'list' && right.t === 'list') return { t: 'list', v: [...left.v, ...right.v] }
      }

      const a = numeric(left, node.op)
      const b = numeric(right, node.op)
      const keepInt = bothInt(left, right)
      const wrap = (value: number) => (keepInt ? int(value) : float(value))
      ...
      switch (node.op) {
        ...
        case '>>': {
          ...
          if (b < 0) throw new PyError('ValueError', 'negative shift count')
 ...
 */
export function evaluate(expression: string, scope: EvalScope): PyValue {
  const names = new Map<string, PyValue>()
  for (const [key, value] of Object.entries(scope)) names.set(key, fromJson(value))
  return evaluateIn(parse(expression), names)
}

/** Evaluate an already-parsed expression against already-typed values. */
export function evaluateIn(expression: Node | string, scope: ReadonlyMap<string, PyValue>): PyValue {
