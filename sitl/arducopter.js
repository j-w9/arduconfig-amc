...
function findWasmBinary() {
  if (Module["locateFile"]) {
    return locateFile("arducopter.wasm");
  }
  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  return new URL("arducopter.wasm", import.meta.url).href;
}
...
var PThread = {
  ...
  allocateUnusedWorker() {
    ...
    worker = new Worker(new URL("arducopter.js", import.meta.url), {
      "type": "module",
      // This is the way that we signal to the node worker that it is hosting
      // a pthread.
      "workerData": "em-pthread",
...
function ___syscall_connect(fd, addr, len, u1, u2, u3) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(6, 0, 1, fd, addr, len, u1, u2, u3);
  try {
    var sock = getSocketFromFD(fd);
    var info = getSocketAddress(addr, len);
...
function ___syscall_getsockname(fd, addr, len, u1, u2, u3) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(12, 0, 1, fd, addr, len, u1, u2, u3);
  try {
    var sock = getSocketFromFD(fd);
...
function ___syscall_getsockopt(fd, level, optname, optval, optlen, unused) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(13, 0, 1, fd, level, optname, optval, optlen, unused);
  try {
    var sock = getSocketFromFD(fd);
...
function ___syscall_listen(fd, backlog, u1, u2, u3, u4) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(15, 0, 1, fd, backlog, u1, u2, u3, u4);
  try {
    var sock = getSocketFromFD(fd);
  ...
  stream_ops: {
    ...
    ioctl(stream, request, argp) {
      if (request == 21531) {
        ...
        (growMemViews(), HEAP32)[((argp) >> 2)] = currentLength;
        return 0;
      }
      return 28;
    },
...
function ___syscall_pipe2(fdPtr, flags) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(20, 0, 1, fdPtr, flags);
  try {
    if (fdPtr == 0) {
...
var doPollSync = (fds, nfds) => {
  ...
  for (var i = 0, pollfd = fds; i < nfds; i++, pollfd += 8) {
    var revents = pollOne((growMemViews(), HEAP32)[((pollfd) >> 2)], (growMemViews(), 
    HEAP16)[(((pollfd) + (4)) >> 1)]);
    if (revents) count++;
    (growMemViews(), HEAP16)[(((pollfd) + (6)) >> 1)] = revents;
  }
  return count;
};
...
var doPollAsync = (fds, nfds, timeout) => new Promise(resolve => {
  ...
  if (count || !timeout) {
    ...
    for (var i = 0, pollfd = fds; i < nfds; i++, pollfd += 8) {
      var stream = FS.getStream((growMemViews(), HEAP32)[((pollfd) >> 2)]);
...
function ___syscall_poll(fds, nfds, timeout) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(21, 0, 2, fds, nfds, timeout);
  try {
...
function ___syscall_recvfrom(fd, buf, len, flags, addr, alen) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(23, 0, 1, fd, buf, len, flags, addr, alen);
  try {
    var sock = getSocketFromFD(fd);
...
function ___syscall_socket(domain, type, protocol, u1, u2, u3) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(28, 0, 1, domain, type, protocol, u1, u2, u3);
  try {
...
function ___syscall_statfs64(path, size, buf) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(30, 0, 1, path, size, buf);
  try {
    assert(size === 88);
...
function __setitimer_js(which, timeout_ms) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(33, 0, 1, which, timeout_ms);
...
function _fd_fdstat_get(fd, pbuf) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(37, 0, 1, fd, pbuf);
  try {
    var rightsBase = 0;
...
function _fd_pread(fd, iov, iovcnt, offset, pnum) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(38, 0, 1, fd, iov, iovcnt, offset, pnum);
...
function _fd_pwrite(fd, iov, iovcnt, offset, pnum) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(39, 0, 1, fd, iov, iovcnt, offset, pnum);
  offset = bigintToI53Checked(offset);
  try {
    if (isNaN(offset)) return 22;
    var stream = SYSCALLS.getStreamFromFD(fd);
...
function _fd_seek(fd, offset, whence, newOffset) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(41, 0, 1, fd, offset, whence, newOffset);
  offset = bigintToI53Checked(offset);
  try {
    if (isNaN(offset)) return 22;
    var stream = SYSCALLS.getStreamFromFD(fd);
...
var _fd_sync = function(fd) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(42, 0, 2, fd);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, pthreadCreateProxied, ___syscall_accept4, ___syscall_bind, ___syscall_chdir, ___syscall_connect, ___syscall_dup3, ___syscall_fcntl64, ___syscall_fstat64, ___syscall_ftruncate64, ___syscall_getdents64, ___syscall_getsockname, ___syscall_getsockopt, ___syscall_ioctl, ___syscall_listen, ___syscall_lstat64, ___syscall_mkdirat, ___syscall_newfstatat, ___syscall_openat, ___syscall_pipe2, ___syscall_poll, ___syscall_readlinkat, ___syscall_recvfrom, ___syscall_renameat, ___syscall_rmdir, ___syscall_sendto, ___syscall_setsockopt, ___syscall_socket, ___syscall_stat64, ___syscall_statfs64, ___syscall_unlinkat, ___syscall_utimensat, __setitimer_js, _environ_get, _environ_sizes_get, _fd_close, _fd_fdstat_get, _fd_pread, _fd_pwrite, _fd_read, _fd_seek, _fd_sync, _fd_write ];
...
var _ardupilot_serial_write = Module["_ardupilot_serial_write"] = makeInvalidEarlyAccess("_ardupilot_serial_write");

var _ardupilot_serial_read = Module["_ardupilot_serial_read"] = makeInvalidEarlyAccess("_ardupilot_serial_read");

var _ardupilot_serial_read_available = Module["_ardupilot_serial_read_available"] = makeInvalidEarlyAccess("_ardupilot_serial_read_available");

var _ardupilot_malloc = Module["_ardupilot_malloc"] = makeInvalidEarlyAccess("_ardupilot_malloc");

var _ardupilot_free = Module["_ardupilot_free"] = makeInvalidEarlyAccess("_ardupilot_free");
...
function assignWasmExports(wasmExports) {
  ...
  assert(typeof wasmExports["ardupilot_serial_write"] != "undefined", "missing Wasm export: ardupilot_serial_write");
  assert(typeof wasmExports["ardupilot_serial_read"] != "undefined", "missing Wasm export: ardupilot_serial_read");
  assert(typeof wasmExports["ardupilot_serial_read_available"] != "undefined", "missing Wasm export: ardupilot_serial_read_available");
  assert(typeof wasmExports["ardupilot_malloc"] != "undefined", "missing Wasm export: ardupilot_malloc");
  assert(typeof wasmExports["ardupilot_free"] != "undefined", "missing Wasm export: ardupilot_free");
  ...
  _ardupilot_serial_write = Module["_ardupilot_serial_write"] = createExportWrapper("ardupilot_serial_write", wasmExports["ardupilot_serial_write"], 3);
  _ardupilot_serial_read = Module["_ardupilot_serial_read"] = createExportWrapper("ardupilot_serial_read", wasmExports["ardupilot_serial_read"], 3);
  _ardupilot_serial_read_available = Module["_ardupilot_serial_read_available"] = createExportWrapper("ardupilot_serial_read_available", wasmExports["ardupilot_serial_read_available"], 1);
  _ardupilot_malloc = Module["_ardupilot_malloc"] = createExportWrapper("ardupilot_malloc", wasmExports["ardupilot_malloc"], 1);
  _ardupilot_free = Module["_ardupilot_free"] = createExportWrapper("ardupilot_free", wasmExports["ardupilot_free"], 1);
